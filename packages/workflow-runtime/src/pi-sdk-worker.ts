...
export class PiSdkWorkerExecutor implements WorkerExecutor {
  private readonly options: { model?: unknown; thinkingLevel?: unknown; createSession?: typeof createAgentSession; resourceLoader?: ResourceLoader };
  constructor(options: { model?: unknown; thinkingLevel?: unknown; createSession?: typeof createAgentSession; resourceLoader?: ResourceLoader } = {}) { this.options=options; }
  async execute(node:NodeDefinition, task:unknown, capsule:Capsule):Promise<Artifact> {
    ...
    const {session}=await create({tools:profile?.tools??[],customTools:[submit],resourceLoader:loader,sessionManager:SessionManager.inMemory(),...(this.options.model ? {model:this.options.model as any} : {}),...(this.options.thinkingLevel !== undefined ? {thinkingLevel:this.options.thinkingLevel} : {})} as any);
