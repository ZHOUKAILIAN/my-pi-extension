...
export interface ActiveReviewAttempt { reviewerIndex:number; workerId:string; cycleId:string; nodeExecutionId:string; attempt:number }
export interface ReviewProgress { cycleId:string; reviewNodeId:string; reviewArtifactKind:Artifact['kind']; reviewedNodeId:string; requiredApprovals:number; reviewerWorkerIds:string[]; approvals:number; nextReviewerIndex:number }
export interface Checkpoint { schemaVersion?:1; runId:string; stage:Stage; at:number; id:string; problem?:string; artifactRefs?:string[]; artifacts?:Artifact[]; pendingDecisionRequest?:string; decisionReference?:string; workflowVersion?:string; policyDigest?:string; nodeExecutionId?:string; logicalNodeExecutionId?:string; recoveryAttempt?:number; activeNodeId?:string; sourceVersion?:string; candidateRevision?:string; reviewCycleId?:string; reviewerIndex?:number; reviewerParticipant?:ReviewParticipant; reviewerWorkerId?:string; /** 仅表示当前未完成的评审尝试；成功评审周期/迁移后必须清除。 */ activeReviewAttempt?:ActiveReviewAttempt; /** 运行是否已发出 run_started（首个 Node 执行后即为 true，恢复时避免重复启动事件）。 */ started?:boolean; /** 评审周期账本：runReview 每次记账一条；change_plan_review 的 DISPOSITION → IMPLEMENTING 门禁
  只认绑定到账本周期（含 quorum / 独立评审者 / 与当前处置配对）的评审 Artifact，直传伪造不再放行。
  其他评审 kind（investigation_review / change_review）也记录，保持跨 session 恢复的评审事实连续。 */ reviewCycles?:ReviewCycleRecord[]; /** 评审 participant 完成后的 WAL checkpoint；用于崩溃后从下一个 participant 继续，而不是重跑已完成者。 */ reviewProgress?:ReviewProgress; /** BLOCKED 解除后回到的目标阶段：记录产生 BLOCKED 时的现场，跨 session 恢复时按此回放，不允许默认回滚到 INVESTIGATING。 */ blockedReturnStage?:Stage; incomplete?:boolean; // 用户决策的审计事实：决策种类（approve 等）、绑定版本与原因进入 checkpoint，避免依赖易丢的决策 Artifact；
