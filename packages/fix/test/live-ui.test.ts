...
test('Pi 0.84.2 CustomEditor contract delegates the previous editor and fences model keys', () => {
  ...
  const theme = { borderColor: (value: string) => value, selectList: {} };
  ...
  assert.ok(editor.render(40).some((line) => line.includes(CURSOR_MARKER)), 'focused editor must emit CURSOR_MARKER');
  editor.focused = false;
  assert.ok(!editor.render(40).some((line) => line.includes(CURSOR_MARKER)), 'unfocused editor must not emit CURSOR_MARKER');
