...
|---|---|---|---|---|
| A：单仓库、单 package、多顶层 extension | 一个 Git repository；一个 Pi package；`build`、`work`、`bug` 等独立顶层 extension；可选普通共享模块 | 同一工作流版本边界；顶层入口独立；联调、安装和升级成本低 | 默认全加载会扩大工具、命令和权限面；需要明确顶层 extension 启用和依赖规则 | **当前推荐** |
...
flowchart LR
    subgraph optionA[方案 A]
        ...
        APkg --> A1[Extension A]
        APkg --> A2[Extension B]
        A1 <--> AShared[Optional Shared Module]
        A2 <--> AShared
    end
...
仓库/package 层面的暂定方向仍是：

> **一个 Git 仓库，先以一个 Pi package 管理多个独立的顶层工作流 extension。**

但 extension 的最终颗粒度必须以 [extension-granularity.md](extension-granularity.md) 的决定为准。当前不把 `solution-review`、`task-delegation`、`code-review`、`verification` 预先定义为顶层 Pi extension；它们先作为 `/build`、`/work`、`/bug` 各自内部的节点候选。

```text
my-pi-extension/
├── package.json
├── extensions/
│   ├── build.ts                   # 顶层工作流 extension，注册 /build
│   ├── work.ts                    # 顶层工作流 extension，注册 /work
│   └── bug.ts                     # 顶层工作流 extension，注册 /bug
├── shared/                        # 仅在重复稳定后提取的无状态普通模块
├── policies/                      # 版本化 Policy
├── skills/                        # 本集合自带 skill（如需要）
├── tests/                         # 各 extension 和共享模块测试
└── docs/                          # 问题、设计、评审和决策文档
```
...
| 判断维度 | 当前方向 | 对当前阶段的意义 |
|---|---|---|
| 工作入口 | `/build`、`/work`、`/bug` 各自是顶层 extension | 入口相互独立，能分别加载、测试和演进 |
| 工作流节点 | 先作为顶层 extension 内部节点 | 避免过早建立子 extension 依赖网 |
| worker 通信 | 顶层 extension 调度独立 worker runtime，并通过主 Pi Session、Artifact、Context Capsule 管理交接 | 子 agent 不直接共享隐式上下文或 session |
| 共享实现 | 只有稳定且必须一致的重复规则才提取普通模块 | 不预设大型 `core` |
| package | 暂定一个 package 承载顶层 extension | 先降低发布和版本协调成本，package 拆分另行评审 |

## 5. 顶层 Extension 的独立性要求

虽然暂时放在一个 package 中，每个顶层工作流 extension 仍必须有独立边界：
...
| 协作 | 仅通过普通共享模块、typed Artifact 或 `pi.events` |
| 依赖 | 显式列出上游和下游依赖 |
| 启停 | 可被配置禁用，或声明为必需依赖 |

`shared` 普通模块的边界：

| 可以进入 `shared` | 不可以进入 `shared` |
|---|---|
| 经至少两个顶层 extension 验证过的无状态 schema、校验或 helper | `/build`、`/work`、`/bug`、`delegate_task` 和其他用户入口的 Pi API 注册；跨 extension 的隐式内存状态 |
...
| 共享模块工程化 | `shared` 是否需要独立测试包或 build step？ |
| 故障隔离 | 一个 extension 加载失败时，其他 extension 是否继续加载？ |
| runtime 共享 | 顶层 extension 是否需要共享同一个 runtime instance，还是各自独立运行？ |
...
仓库/package 的暂定方向是“单仓库、单 Pi package、多顶层工作流 extension”，但 extension 颗粒度仍以 [extension-granularity.md](extension-granularity.md) 的评审结论为准。状态仍是 `ALIGNING`；在 extension 颗粒度和 `01-extension-catalog` 完成之前，不创建具体 extension 的实现代码。
