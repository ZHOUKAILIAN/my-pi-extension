...
| 改变正式需求 / 方案 / 架构 / 验收 | 升级为 Core 的 Proposal + Independent Review + Adoption Decision |
...
Controller 根据已校验 Artifact 生成最终报告，不把 Worker 自由文本总结直接当报告事实。报告使用固定结构，没有事实的字段写“无”或“尚未确认”，不得省略或以泛化措辞掩盖。
...
## 现象与问题确认
- 用户报告的现象 / 预期行为及依据 / 实际行为 / 确认结果（已复现 / 间接证据 / 未能确认）/ 影响范围

## 根因分层与因果证据链
- 最终根因 / 停止调查的理由 / 时间范围与时区

| 时间 | 事件与状态变化 | 原因层级 | 证据与来源 | 对因果链的证明作用 |
| --- | --- | --- | --- | --- |
| <T0> | <前置条件> | 前提 | <来源> | <为何路径被触发> |
| <T1> | <触发事件> | 代码 / 配置 / 数据原因 | <来源> | <问题从何开始> |
| <T2> | <状态或调用传导> | 直接故障点 | <来源> | <如何到达故障点> |
| <T3> | <用户可见现象> | 用户现象 | <来源> | <为何形成报告现象> |
| <T4> | <修复后同一路径对照> | 根因验证 | <来源> | <为何证明链路被切断> |

- 因果链摘要 / 已排除的候选原因 / 证据缺口与未验证假设（没有写“无”）

## 影响面与已采取的处置
- 修复触及的链路 / 可能连带影响的既有场景 / 未覆盖的影响面 / 处置 / 相关文件或对象 / 处置范围（最小必要及扩大原因）

## 验证结果
- 原始问题验证 / 回归与兼容性验证 / 已执行命令或检查 / 验证结论

## 风险与后续
- 剩余风险 / 需要用户决定或协助（没有写“无”）
```
...
| 结构化原因 | 人工 `request-changes` / `reject` / 重开 / 回滚 / 后验问题都有结构化原因 |
...
质量指标（流程 10 步 / 质量漏斗 / 自动化准入漏斗）、减少人工的建议门槛、完整结构化事件表已归档，第一版不实现：见 [Fix 自动化演进指标](../../归档/研究/2026-08-28-fix-automation-evolution-metrics.md)。第一版只做 5 步公开漏斗，先把 `/fix → 事件 → Worker → D1 → 公开 API` 基建打通。
