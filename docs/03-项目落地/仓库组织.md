...
├── subagent Pi package → subagent child-session Extension
...
flowchart TB
    ...
    fix --> contracts
    subagent[subagent package] -.->|optional peer + safe dynamic adapter| codex[codex-usage-status interop]
    subagent -. standalone child dispatcher .-> pi[Pi CLI / SDK]
    runtime --> contracts
```
...
- `@pi/codex-usage-status` 通过 `./interop` public subpath 提供 Fast 协议；它的 Pi `src/index.ts` extension entry 与该 library-like export 分开维护。
- `@pi/subagent` 将 `@pi/codex-usage-status` 声明为 optional peer，并通过安全 dynamic adapter 按 producer 是否存在启用 Fast；producer 缺失时独立安装仍可加载运行且 Fast 安全 Off。
...
当前仓库已使用 `packages/fix` / `@pi/fix` / `/fix`，尚无 `packages/feature`。`packages/codex-usage-status` 是独立安装、启用和回滚的 provider-status extension；它不属于 Workflow Runtime，也不依赖 `workflow-runtime` 或 `workflow-contracts`，当前源码已实现。`packages/subagent` 是独立的 child-session dispatcher package，不创建 Workflow Run/Artifact/Stage；本轮 package 已落地但未修改用户 settings 或替换 local extension。Fix 完整治理流程与当前原型的功能差距见[第二层实现地图](../02-产品实现/README.md#已知-l1--l2-drift)。
