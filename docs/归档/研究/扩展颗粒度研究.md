...
> 归档说明：这是历史研究材料，不是当前架构的正式真理源。当前结论以[仓库组织方案](../../03-项目落地/仓库组织.md)为准。

- 历史状态：`ALIGNING`
- 当前正式结论：[仓库组织方案](../../03-项目落地/仓库组织.md)
- 当前扩展集合：[扩展目录](../../03-项目落地/扩展目录.md)
...
这些边界的候选组合、package 方案和评估过程统一放在当前正式的[仓库组织方案](../../03-项目落地/仓库组织.md)。本文件只保留历史分析，不再形成第二个架构真理源。

## 5. 颗粒度选型：本文件只记录运行时事实，不提前定案

Node 是否成为 extension，必须与 package 选型分开判断。完整候选方案的当前结论统一以[仓库组织方案](../../03-项目落地/仓库组织.md)为准。
...
总体架构选型当前以[仓库组织方案](../../03-项目落地/仓库组织.md)为准：
