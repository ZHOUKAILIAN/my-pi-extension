...
| `disposition -.-> 需要用户或外部决定 -> WAITING_FOR_USER`（L1 虚线边） | 已实现：`DISPOSITION->WAITING_FOR_USER` guard 边要求 `dispositionType` 为 `wait_decision`/`external_action`；continuation 先停 `WAITING_FOR_USER`（`pendingDecisionKind` 分别为 `disposition_decision`/`external_action_completion`），用户决定后经 `continue_disposition` 回 `DISPOSITION`（等待决定）或按 `requiresRepositoryChange` 回 `VERIFYING`/`DISPOSITION`（外部动作完成）。Runtime 仅在 checkpoint 持久化 `pendingDecisionKind` 并恢复（未知值 `CHECKPOINT_STATE_INCONSISTENT` fail-closed；legacy 缺字段回退验证推导；restore 交叉校验处置等待与等待种类配对，不配对 fail-closed），业务路由保留在 Fix 包；恢复 WAITING 校验接受 disposition 支撑的等待现场 | 已实现（防御纵深：`DISPOSITION->IMPLEMENTING`/`->VERIFYING` 拒绝 `wait_decision`/`external_action`） |
| `PendingDecisionKind` 四值枚举与 `continue_disposition` 的 stage 映射由 Fix Extension 拥有（Fix 专属等待语义） | `PendingDecisionKind` 类型定义在 `packages/workflow-contracts`；持久化/恢复/未知值校验/`continue_disposition` 的 stage 映射（`disposition_decision`→DISPOSITION；`external_action_completion`+无仓库变更→VERIFYING、声明仓库变更→DISPOSITION）都在 `packages/workflow-runtime`（`applyTransition`/`restore`/`decideCore`），Fix 包仅在使用点 `setPendingDecisionKind`/`getPendingDecisionKind` 声明与消费。这些 Fix 业务语义仍在 shared Runtime，owner 待拆 | 后续迭代：通过 WorkflowDefinition 显式 capability/hook（如 `pendingDecisionKinds`/`decisionRouting`）收敛或迁回 `packages/fix`；当前如实记录 owner drift，不声称已迁回 Fix 包 |
| `verification -.-> 缺少验证条件 -> BLOCKED`（L1 虚线边） | 已实现三向分流：`verification.accepted=false` 按 `failure.kind` 路由 —— `implementation`→回流 `IMPLEMENTING`；`configuration`→`WAITING_FOR_USER`（无仓库变更的配置问题不回 IMPLEMENTING）；`external_condition`→`BLOCKED`（`recordBlocker`，returnStage=VERIFYING）。契约违规仍为重试→暂停。契约层强制 `accepted=false` 必须携带结构化 `failure`（`MISSING_VERIFICATION_FAILURE`），杜绝静默回流 | 配置类等待的重新验证出口已实现：`continue_verification`→回 `VERIFYING`（仅 `pendingDecisionKind='configuration_wait'` 或验证推导的配置类失败时可用，否则 `CONTINUE_VERIFICATION_NOT_AVAILABLE`） |
